...
──────────────────────────────  OUTPUT FORMAT  ─────────────────────────────
Return **only** valid JSON whose keys match the `PytorchCodeWithTests`
Pydantic model:

{
  "format_pt_code": "<implementation-only section, no tests>",
  "tests": "<tests section, no implementation>",
  "pt_code_runs": <true | false>,
  "entrypoint": "<function_name>"
}

• **No** Markdown code fences or extra keys.  
• `pt_code_runs` must be `true` *only if* you have actually executed the
  file and all assertions pass on your end.  
• `entrypoint` must match the function name and the test name
  (`test_<entrypoint>`).

Example skeleton of a correct response:

{
  "format_pt_code": "import torch\\nDEVICE = ...\\n\\n
                     def addmm(...):\\n    ...",
  "tests": "import torch\\nfrom <module> import addmm\\n...\\n
            def test_addmm(): ...\\n\\n
            test_results = test_addmm()\\nprint(test_results)",
  "pt_code_runs": true,
  "entrypoint": "addmm"
