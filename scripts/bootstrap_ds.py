...
class Args:
    model: str = "gpt-4.1"
    weave_project: str = "grpo-cuda/boostrap_triton"
...
system_prompt = """We are generating a PyTorch/Triton pairs dataset. We want functions that have exactly the same funcionalities.
...
Some ideas, think about fusing operations, specific types of matrices like diagonals, transposes, reshapes, noising, etc...

Let's add some simple attention operations but with realistic implementations.

Here you have an example from the tutorials, get inspired by it, but make something wayyyy simpler keeping the format of our dataset.
...
configs = [
    triton.Config(dict(BLOCK_M=BM, BLOCK_N=BN), num_stages=s, num_warps=w, pre_hook=_host_descriptor_pre_hook) for BM in [64, 128, 256] for BN in [64, 128] for s in NUM_STAGES_OPTIONS for w in [4, 8]
]
...
@weave.op
def generate_one_more_row(ds):
    rows = ds.to_list()[-args.rows:]
...
for i in range(args.N):
    print(f"Generating row {i+1} of {args.N}")
    extra_row = generate_one_more_row(ds)
    print("="*100)
    print(extra_row.pt_code)
    print("-"*100)
    print(extra_row.triton_code)
    print("="*100)
    ds = ds.add_item(extra_row.model_dump())
