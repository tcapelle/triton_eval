...
# agent.run(user_prompt=f"Here is the file that needs fixing:\n#File:\n{broken_file}")
...
def fix_code(row):
    if row["test_cuda_passing"]:
        ...
        try:
            agent = Agent(system_message=system_prompt, silent=True)
            agent.run(user_prompt=f"Here is the code that needs fixing:\n#Code:\n{pytorch_code}")
        except Exception as e:
            print(f"Error: {e}")
            return {"pytorch_code_with_test_cases_fixed": pytorch_code, "test_cuda_passing": False, "fixed": False}
        return {"pytorch_code_with_test_cases_fixed": agent.response.final_response, "test_cuda_passing": True, "fixed": True}
...
def fix_test_cases(dataset):
    ...
    # dataset = dataset(fix_code, num_proc=12)
